...
@app.route('/coffee_chatbot', methods=['POST'])
def coffee_chatbot():
    try:
        data = request.json
        if data is None:
        ...
        user_input = data.get('ai_input')  # ✅ Corrected key

        print(f"User input: {user_input}")

        # Define AI generation logic
        def generate():
            global ai_generated_text
            ...
            for chunk in client.models.generate_content_stream(
                ...
                ai_generated_text += chunk.text

            print(f"Generated Text: {ai_generated_text}")

            # Save to Firestore
            doc_ref = db.collection("ai_ordercoffee_output").document()
            doc_ref.set({
